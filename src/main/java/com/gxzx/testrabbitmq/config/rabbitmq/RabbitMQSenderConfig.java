 ...
 * 
 *                                                                                                                                                   
1、撮合引擎支持一主多从，建议配置至少一主二从以上，撮合系统主从切换的时间不超过3秒。即使发生主撮合引擎宕机的极端情况时，不会影响用户的挂单、撤单、提现、转账等委托请求。但会影响行情、盘口数据的推送，延迟不超过3秒。
2、支持机构或专业用户的高频量化交易，下单首先经过基于Redis内存的账户记账，在撮合之前，账户资金变更与订单等不入库持久化，同时做到确保数据最终一致性。
3、挂单的系统延迟时间在10毫秒以内。撤单按用户分两种情况，机构或专业用户采用异步撤单机制，请求延迟在10毫秒以内直接返回受理结果，撤单结果需要机构用户自己主动查询。普通用户撤单，响应延迟在100毫秒内内，返回撤单是否成功。
4、撮合服务基于本地内存，并进行流水线撮合，单笔撮合的时间开销能控制在10微妙以内，基于MQ做定序的撮合引擎的吞吐量可以达到每秒5000笔。我们正在研发另一方案，撮合性能预计可以提升到每秒10万，并且不需要采购专用服务器和特殊付费软件，系统能够部署在云平台通用服务器。
...
8、挂单请求发送到网关，网关调用基于Redis的账户记账扣款后，将委托单发送到MQ，但只发送一次消息，如果发送失败了并不再重发，只记录异常事件，等待定期做redis内存账户回滚操作。
